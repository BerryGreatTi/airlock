...
│                    │                                                │
│ WORKSPACES         │  [Terminal] [Secrets] [Containers] [Diff] [Settings]
│                    │  ──────────────────────────────────────────────│
│ ● my-api       ▶  │                                                │
...
| Tab | Role | Default? |
|-----|------|----------|
| Terminal | Splittable shells inside container | Yes |
| Secrets | .env editing, encryption status | |
| Containers | Agent/proxy/network status, proxy activity log | |
| Diff | git diff side-by-side (unchanged) | |
| Settings | Per-workspace + global config (unchanged) | |
